...
# Use pre-prompt for context
floimg ai edit \
  --image product.jpg \
  --pre-prompt "This is a product photo. " \
  --prompt "Make it look more premium with dramatic lighting" \
  --out premium.jpg
```
...
# Detailed prompt
floimg ai generate \
  --prompt "A photorealistic mountain lake at sunrise, mist rising from the water, reflection of snow-capped peaks" \
  --out landscape.png
